...
public class SwerveInverseKinematics {
	...
	//private List<UnwrappableTranslation2d> moduleRelativePositions = Constants.kModulePositions.stream().map(Translation2d::unwrap).collect(Collectors.toList());
	...
	public void setCenterOfRotation(UnwrappableTranslation2d center) {
		...
		for (int i = 0; i < kNumberOfModules; i++) {
			UnwrappableTranslation2d position = Constants.kModulePositions.get(i).unwrap().translateBy(center.inverse());
			positions.add(position.wrap());
		...
		for (int i = 0; i < kNumberOfModules; i++) {
			UnwrappableTranslation2d position = positions.get(i).unwrap();
			positions.set(i, position.scale(1.0 / maxMagnitude).wrap());
		}
	...
	public List<Translation2d> updateDriveVectors(Translation2d translationalVector, double rotationalMagnitude,
		...
		for (int i = 0; i < kNumberOfModules; i++) {
			UnwrappableTranslation2d driveVector = driveVectors.get(i).unwrap();
			driveVectors.set(i, driveVector.scale(1.0 / maxMagnitude).wrap());
		}
		return driveVectors;
	}
